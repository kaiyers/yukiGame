...
Page({
  ...
  onLoad:function(options){
    if (options.backisCollected){
      that.setData({
        backisCollected: options.backisCollected
      })
    }
    if (options.backgoodid) {
      that.setData({
        backgoodid: options.backgoodid
      })
    }
    var that =this
  ...
  getMsg:function(){
    var that = this
    wx.request({
      url: util.Apis + '/h5/h5labor/laborGoodList',
      data: {
  ...
  pullUpLoad: function () {
    console.log(this.data.maxIndex, this.data.indenx)
