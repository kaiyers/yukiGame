...
Page({
  ...
  onShow(){
    this.getData()
    this.setData({
      infomationIcon: app.globalData.infomationIcon
    })
  },
  getData:function(){
    var that = this;
        ...
        if (that.data.isfromCar) {
          wx.request({
            ...
            success: function (res) {
              ...
              //处理地址
              datas.addresses.forEach((value, index) => {
                value.isSle = false;
                if (value.isAvalible) {
                  value.isSle = true;
                }
             ...
             that.setData({
                address: datas.addresses,
                addInfo: datas.orderBean.address,
                orders: datas.orderBean.orderItem,
                orderinfo: datas.orderBean.order,
                needpay: datas.orderBean.order.needPay,
                discount: datas.orderBean.orderItem[0].vip ? datas.orderBean.orderItem[0].vip.vipDiscount * 10 : 10,
      ...
      success: function (res) {
        ...
        wx.request({
          ...
          data: {
            'userId': that.data.uid,
            //个人-地址信息
            'addrId': that.data.addInfo.addrid,
            'receiveName': that.data.addInfo.name,
            'receivePhone': that.data.addInfo.phone,
            'receiveAddress': that.data.addInfo.address,
            //商品信息
          ...
          success: function (res) {
            ...
            var jdjd = that.data.infomationIcon++
            console.log("jdjd", jdjd + 1)
            app.globalData.infomationIcon = (jdjd + 1).toString()
