...
class LayaSample{
    private myPage:ui.FBGameUI;
    private FB:FBInstant;
    private playerId:String;
    constructor(){
        //实例界面
        this.myPage = new ui.FBGameUI();
        this.myPage.btnShare.on(laya.events.Event.CLICK,this,this.onBtnShare);
        this.myPage.btnLogout.on(laya.events.Event.CLICK,this,this.onBtnLogout);
        this.myPage.btnSave.on(laya.events.Event.CLICK,this,this.onBtnSave);
        this.myPage.btnLoading.on(laya.events.Event.CLICK,this,this.onBtnLoading);
        this.myPage.btnFriend.on(laya.events.Event.CLICK,this,this.onBtnFriend)
        this.myPage.btnChoose.on(laya.events.Event.CLICK,this,this.onBtnChoose)
        //FB初始化完成
        this.FB = laya.utils.Browser.window.FBSDK;
        //此值未隐藏ID，这里为了方便直接写到此处，用户请通过服务器传递，
        this.FB.initializeAsync(this.initializeAsync.bind(this));
     ...
     * 
     */		
    private onBtnChoose():void
    {			
        alert("调用了'选择游戏场景'接口,此项目为模拟项目,具体效果请在facebook环境下调试");
        this.FB.context.chooseAsync(this.playerId,function():void{
     ...
     * 
     */		
    private onBtnFriend():void
    {			
        alert("调用了'同玩好友'接口,此项目为模拟项目,具体效果请在facebook环境下调试");
        //获取同玩的好友，关系链
        this.FB.player.getConnectedPlayersAsync(function(data):void{
            for(var i:number = 0;i<data.length;i++){
                var a:ConnectedPlayer = data[i];
                console.log("getConnectedPlayersAsync:"+"ID:"+this.a.getID()+"Name:"+this.a.getName()+"photo:"+this.a.getPhoto());//[ConnectedPlayer]
            }
            //获取下某个好友的ID
     ...
     * 
     */		
    private onBtnLoading():void
    {			
        alert("调用了'获取远程数据'接口,此项目为模拟项目,具体效果请在facebook环境下调试");
        this.FB.player.getDataAsync(["name","data"],function (data):void {
    ...
    {			
        ...
        //上传自定义数据到平台
        this.FB.player.setDataAsync({name:"setDataAsync",data:"2333333"},function ():void {
            //从平台获取自定义数据
            alert("setDataAsync");
            //立即更新数据到平台，谨慎使用，
            this.FB.player.flushDataAsync(function ():void {
    ...
    private onBtnShare():void
    {
        ...
        var payload:SharePayload = new laya.utils.Browser.window.SharePayload();
        ...
        this.FB.shareAsync(payload,function():void{
            alert("分享");
        });
    }
}
...
//加载界面资源
Laya.loader.load([{url:"res/atlas/comp.atlas",type:laya.net.Loader.ATLAS}],new laya.utils.Handler(this,this.completed));
