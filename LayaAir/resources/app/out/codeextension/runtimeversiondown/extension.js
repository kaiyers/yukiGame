function activate(context)
{
	...
	window.RuntimePanel = function RuntimePanel(type)
	{
		if (!_appversionsserver) 
		{
			window[type]()
			return
		}
		...
		{
			var localversion = fs.readFileSync(path.join(__dirname, "laya_native_config.json"), "utf-8");
		...
		if (localversion.version < _appversionsserver.version)
		{
		...
		___callback = window[type];
		if (window.runtimePanel)
		{
			document.body.appendChild(window.runtimePanel);
		}
		else
		{
			window.runtimePanel = cretePanel(localversion);
